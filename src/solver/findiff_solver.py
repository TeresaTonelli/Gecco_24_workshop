...
class fd_solve():
    ...
    def __call__(self, u, t):
        ...
        du_dt = np.concatenate(
            (du_dt_bnd[:, 0:self.args.max_deriv - 1], du_dt_int, du_dt_bnd[:, self.args.max_deriv - 1:]), axis=-1)

        return tf.convert_to_tensor(du_dt, dtype=tf.float32)   #du_dt
    ...
    def boundary(self, t, u_t, du_dt_int):
        ...
        def rhs_entry(eqn_id, bnd):
            ...
            b = - np.einsum('b, cb -> c',
                            self.full_vander[eqn_id][bnd, self.args.max_deriv - 1:-(self.args.max_deriv - 1)],
                            du_dt_int)
            return b
        ...
        for bnd in [0, -1]:
            for i in range(self.args.max_deriv):
                ...
                B[k] = tf.expand_dims(tf.stack(B[k], axis=-1), axis=-2)                #tf, tf
                k += 1
        ...
        b_tmp.append(self.args.dright_bv_dt + (b[self.args.max_deriv]) * self.args.dbc_r + (
        b[self.args.max_deriv + 1]) * self.args.nbc_r)

        for i in range(self.args.max_deriv + 2, 2 * self.args.max_deriv):
            B_tmp.append(B[i])
            b_tmp.append(b[i])

        B = tf.concat(B_tmp, axis=-2)     #tf
        b = tf.stack(b_tmp, axis=-1)      #tf

        du_dt_bnd = tf.einsum('ab, cb ->ca', tf.linalg.pinv(B), b)       #tf, tf
