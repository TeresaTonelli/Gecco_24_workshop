from solver.DDE_solver import ddeinttf
import solver.findiff_general as fdgen
...
def kalecki_solve(args, u0=None, t=None, alg_name='dopri5', nsteps=-1):
    ...
    sol_obj = kalecki.kalecki_rhs(args=args, deriv_obj=deriv_obj, grid_obj=grid_obj)
...
class custom_plot:
    ...
    def plot_3d(self, *pred_y, epoch=0):
        ...
        if epoch < 0 or self.restart == 1:
            ...
            ax_u_nn.plot_surface(self.X, self.T, tf.squeeze(pred_y[0][:, :, 0:self.args.state_dim], axis=1).numpy(),
                                 cmap=cm.coolwarm,
                                 linewidth=0, antialiased=False)
            ...
            ax_u_diff_nn.plot_surface(self.X, self.T,
                                      tf.squeeze(self.true_y - pred_y[0][:, :, 0:self.args.state_dim], axis=1).numpy(),
                                      cmap=cm.coolwarm,
                                      linewidth=0, antialiased=False)
    ...
    def plot(self, *pred_y, epoch=0):
        ...
        plot = ax_u_diff.contourf(self.X, self.T, tf.abs(tf.squeeze(self.true_y - self.y_no_nn, axis=1)).numpy(),
        ...
        if epoch >= 0 or self.restart == 1:
            ...
            plot = ax_u_nn.contourf(self.X, self.T, tf.squeeze(pred_y[0][:, :, 0:self.args.state_dim], axis=1).numpy(),
                                    cmap=cm.coolwarm,
                                    antialiased=False, levels=np.linspace(0, 0.5, 20), extend='min')
            ax_u_nn.set_xlim(self.x[0], self.x[-1])
            ax_u_nn.set_ylim(self.t[0], self.t[-1])
            ...
            plot = ax_u_diff_nn.contourf(self.X, self.T, tf.abs(
                tf.squeeze(self.true_y - pred_y[0][:, :, 0:self.args.state_dim], axis=1)).numpy(),
                                         cmap=plt.get_cmap('coolwarm'),
                                         antialiased=False, levels=np.linspace(0, 0.25, 20))
            ax_u_diff_nn.set_xlim(self.x[0], self.x[-1])
