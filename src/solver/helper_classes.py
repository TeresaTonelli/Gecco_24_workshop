...
class save_dir():
    ...
    def __call__(self, script_name):
        ...
        move(os.path.join(self.testcase_dir, script_name + ".py"),
             os.path.join(self.args.model_dir, "orig_run_file.py"))
...
def integrate_simp_cumsum(y, x, axis=-1):
    ...
    y_avg = 0.5 * (np.take(y, list(range(0, y.shape[axis] - 1)), axis=axis) + np.take(y, list(range(1, y.shape[axis])),
                                                                                      axis=axis))
