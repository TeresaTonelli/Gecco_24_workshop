...
class ddeVar:
    ...
    def __init__(self, g, tc=0., sign_flag=True):
        ...
        #self.interpolator = scipy.interpolate.interp1d(
         #   np.array([tc - 1, tc]),  # X
          #  np.vstack([self.g(tc).flatten(), self.g(tc).flatten()]),  # Y
           # kind="linear",
            #bounds_error=False, axis=0,
            #fill_value=self.g(tc).flatten()
        #)
    ...
    def __call__(self, t=0.):
        ...
        return self.g(t) if (t <= self.tc and self.sign_flag) or (
                    t >= self.tc and not self.sign_flag) else tf.convert_to_tensor(
            np.reshape(self.interpolator(t), self.data_shape), tf.float32)

        #return self.g(t) if (t <= self.tc and self.sign_flag) or (
         #       t >= self.tc and not self.sign_flag) else np.reshape(self.interpolator(t), self.data_shape)
...
class dde(scipy.integrate.ode):
    ...
    def __init__(self, f, jac=None):
        def f2(t, y, args):
            ...
            #return np.reshape(f(self.Y, t, *args), [-1])
    ...
    def set_initial_value(self, Y):
        ...
        #scipy.integrate.ode.set_initial_value(self, Y(Y.tc).flatten(), Y.tc)
...
def ddeinttf(func, g, tt, fargs=None, alg_name='dopri5', nsteps=-1):
    ...
    #dde_.set_initial_value(ddeVar(g, tt[0], np.sign(tt[-1] - tt[0])))
    ...
    #results = np.array(results)    #anche questa la ho aggiunta io
    return tf.concat([tf.expand_dims(g(tt[0]), 0), tf.convert_to_tensor(results, tf.float32)], 0)
    #return np.concat([np.expand_dims(g(tt[0]), 0), results], 0)